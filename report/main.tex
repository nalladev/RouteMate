...
\justify\large This snippet shows how a passenger creates a ride request. The backend validates the request, calculates the fare based on distance, and stores it in the database.
...
export async function handleRequest(request: Request) {
  ...
  if (driverId === user.Id) {
    return Response.json({ error: 'Cannot request from yourself' }, 
      { status: 400 });
  }

  // Calculate fare based on actual route distance from OSRM
  const distance = await getRouteDistance(pickupLocation, destination);
  const fare = calculateFare(distance);

  // Validate passenger has sufficient balance
  const currentUser = await getDocumentById('users', user.Id);
  if (currentUser.WalletBalance < fare) {
    return Response.json(
      { error: `Insufficient balance. Required: Rs ${fare}, Available: Rs ${currentUser.WalletBalance}` },
      { status: 400 }
    );
  }

  // Generate OTP for ride verification (10 minute expiry)
  ...
  return Response.json({ requestId, expiresAt: expiresAt.toISOString() });
}
\end{verbatim}

\subsection{Route Matching Logic}

\justify\large The ride matching algorithm identifies suitable drivers based on proximity to the passenger's pickup and destination locations, using geometric distance calculations.
...
function isPointNearRoute(
  ...
  // Calculate perpendicular distance from point to route segment
  ...
  let xx = routeStartLat;
  let yy = routeStartLng;
...
// Filter drivers: pickup must be near route OR near driver location
// AND destination must be near route OR near driver's destination
const pickupCompatible = 
  isPointNearRoute(...driverRoute) || 
  calculateDistance(passengerPickup, driverLocation) <= 5;
  
const destinationCompatible = 
  isPointNearRoute(...driverRoute, passengerDest) || 
  calculateDistance(passengerDest, driverDestination) <= 5;

if (pickupCompatible && destinationCompatible) {
  // Include driver in results
}
\end{verbatim}

\subsection{OTP Verification for Ride Start}

\justify\large To ensure secure ride initiation, an OTP verification mechanism is used between the driver and passenger. Only the driver can verify the OTP after passenger entry.
...
\justify\large The following snippet demonstrates atomic wallet transaction handling during ride completion using Firestore transactions. The transaction ensures consistency with double validation, transaction records, and reward point allocation.
...
const result = await runTransaction(async (transaction) => {
  ...
  const passengerBalance = passengerDoc.data().WalletBalance;
  const driverBalance = driverDoc.data().WalletBalance;

  // Critical validation: prevent negative balance
  if (passengerBalance < fare || passengerBalance - fare < 0) {
    transaction.update(connectionRef, { PaymentStatus: 'failed' });
  ...
  transaction.update(passengerRef, { 
    ...
    PassengerRewardPoints: (passengerDoc.data().PassengerRewardPoints || 0) + 10
  });
  ...
  return { success: true, passengerBalance: newPassengerBalance };
});
\end{verbatim}

\subsection{Real-Time Location Update}

\justify\large This snippet shows how user location is continuously tracked and updated in Firestore with intelligent debouncing to reduce database writes by over 90\%.

\begin{verbatim}
// Frontend: Debounced Location Updates
  ...
  Location.watchPositionAsync(
    ...
    (location) => {
      ...
      if (!lastUpdate) {
        ...
        const distanceMoved = calculateDistance(lastUpdate.location, newLocation);
        
        // Update if 30 seconds passed OR moved 100+ meters
        if (timeSinceLastUpdate >= LOCATION_UPDATE_INTERVAL_MS ||
            distanceMoved >= LOCATION_UPDATE_MIN_DISTANCE_M) {
          shouldUpdate = true;
        }
      }
      
      if (shouldUpdate) {
        lastLocationUpdateRef.current = { location: newLocation, timestamp: now };
        api.updateLocation(newLocation.lat, newLocation.lng);
      }
    }
